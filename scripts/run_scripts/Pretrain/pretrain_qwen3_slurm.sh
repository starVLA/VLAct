...
export WANDB_ENTITY=${WANDB_ENTITY:-your_name}
...
export Framework_name freeze_module_list base_vlm config_yaml data_root_dir run_root_dir data_mix
export vlm_data
export image_size_buckets heads head_loss_weights run_id
export GPUS_PER_NODE NNODES MASTER_ADDR MASTER_PORT NUM_PROCESSES

srun --nodes $NNODES --ntasks $NNODES --ntasks-per-node 1 bash -c 'accelerate launch \
