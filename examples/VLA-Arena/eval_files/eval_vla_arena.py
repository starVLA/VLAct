...
class Args:
    ...
    skip_task_ids: str = ""
    """Comma-separated task ids to skip for this suite/level, e.g. '3'."""
...
def _run_episode(
    ...
    try:
        while t < max_steps + args.num_steps_wait:
            ...
            delta_action = np.concatenate([world_vector, rotation_delta, gripper], axis=0).astype(np.float32)
            if not np.all(np.isfinite(delta_action)):
                logger.warning(f"Non-finite action before env.step: {delta_action}")
                delta_action = np.nan_to_num(delta_action, nan=0.0, posinf=1.0, neginf=-1.0)

            clipped_action = np.clip(delta_action, -1.0, 1.0)
            if not np.allclose(delta_action, clipped_action):
                logger.warning(
                    "Clipped action before env.step: "
                    f"raw={np.array2string(delta_action, precision=4)} "
                    f"clipped={np.array2string(clipped_action, precision=4)}"
                )
            delta_action = clipped_action
...
def _parse_skip_task_ids(skip_task_ids: str) -> set[int]:
    if not skip_task_ids:
        return set()
    return {
        int(item.strip())
        for item in skip_task_ids.split(",")
        if item.strip()
    }
...
def eval_vla_arena(args: Args) -> dict:
    ...
    for suite_name in suite_names:
        ...
        skipped_task_ids = _parse_skip_task_ids(args.skip_task_ids)

        for task_id in tqdm.tqdm(range(num_tasks), desc=f"{suite_name} tasks"):
            if task_id in skipped_task_ids:
                logger.warning(
                    f"Skipping task level={task_level} id={task_id} due to --args.skip-task-ids"
                )
                continue
