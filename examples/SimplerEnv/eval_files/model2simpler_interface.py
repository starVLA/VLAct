...
def read_norm_stats(policy_ckpt_path: Path) -> dict:
    policy_ckpt_path = Path(policy_ckpt_path)
    if not policy_ckpt_path.is_file():
        raise FileNotFoundError(f"Pretrained checkpoint `{policy_ckpt_path}` does not exist.")
    if policy_ckpt_path.suffix != ".pt":
        raise ValueError(f"Expected a .pt checkpoint, got `{policy_ckpt_path}`.")
...
def read_include_state_from_train_script(policy_ckpt_path: Path) -> bool:
    run_dir = Path(policy_ckpt_path).parents[1]
    train_script = run_dir / "train_state.sh"
    if not train_script.exists():
        return False

    text = train_script.read_text()
    match = re.search(r"--datasets\.vla_data\.include_state\s+([^\s\\]+)", text)
    if not match:
        return False
    return match.group(1).lower() in {"1", "true", "yes", "y", "on"}
...
class ModelClient:
    def __init__(
        ...
        self.include_state = read_include_state_from_train_script(Path(policy_ckpt_path))
        ...
        norm_stats = read_norm_stats(Path(policy_ckpt_path))
        self.action_norm_stats = self.get_action_stats(self.unnorm_key, norm_stats=norm_stats)
        self.state_norm_stats = self.get_state_stats(self.unnorm_key, norm_stats=norm_stats)
        print(f"*** include_state from train_state.sh: {self.include_state} ***")
        ...
        if self.include_state:
            ...
            if state is None:
                if not self._warned_missing_state:
                    print("[WARN] include_state=True in train_state.sh, but no state was provided.")
                    self._warned_missing_state = True
            else:
                example["state"] = self.normalize_state(state)
    ...
    @staticmethod
    def get_action_stats(unnorm_key: str, norm_stats: dict) -> dict:
        """
    ...
    @staticmethod
    def get_state_stats(unnorm_key: str, norm_stats: dict) -> Optional[dict]:
        return norm_stats.get(unnorm_key, {}).get("state")
    ...
    def normalize_state(self, state: np.ndarray) -> np.ndarray:
        ...
        if state.shape[-1] != q01.shape[-1]:
            raise ValueError(f"State dim mismatch: got {state.shape[-1]}, expected {q01.shape[-1]}")

        normalized = np.zeros_like(state, dtype=np.float32)
        mask = q01 != q99
        normalized[..., mask] = (state[..., mask] - q01[mask]) / (q99[mask] - q01[mask])
        normalized[..., mask] = normalized[..., mask] * 2.0 - 1.0
        normalized[..., ~mask] = state[..., ~mask]
        normalized = np.clip(normalized, -1.0, 1.0)

        gripper_idx = normalized.shape[-1] - 1
        threshold = (
            self.state_norm_stats["min"][gripper_idx]
            + self.state_norm_stats["max"][gripper_idx]
        ) / 2.0
        normalized[..., gripper_idx] = (state[..., gripper_idx] > threshold).astype(np.float32)
