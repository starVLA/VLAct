...
# ---------------------------------------------------------------------------
# DataConfig — Agilex (DOMINO, action_indices=16/32)
# ---------------------------------------------------------------------------
...
class AgilexWrapData32Config(AgilexData32Config):
    def transform(self):
        return ComposedModalityTransform(transforms=[
            StateActionToTensor(apply_to=self.state_keys),
            StateActionTransform(
            ...
            StateActionTransform(
                apply_to=self.action_keys,
                normalization_modes={
                    "action.left_joints": "wrap", "action.right_joints": "wrap",
                    "action.left_gripper": "binary", "action.right_gripper": "binary",
                },
            ),
        ])
...
# ---------------------------------------------------------------------------
_CLEAN_DYNAMIC = [(f"Clean_Dynamic/{t}", 1.0, "robotwin32") for t in DOMINO_35_TASKS]
_RANDOM_DYNAMIC = [(f"Random_Dynamic/{t}", 1.0, "robotwin32") for t in DOMINO_35_TASKS]
_CLEAN_DYNAMIC_WRAP = [(f"Clean_Dynamic/{t}", 1.0, "robotwin_wrap32") for t in DOMINO_35_TASKS]
_RANDOM_DYNAMIC_WRAP = [(f"Random_Dynamic/{t}", 1.0, "robotwin_wrap32") for t in DOMINO_35_TASKS]
_CLEAN_STATIC = [(f"Clean/{t}", 1.0, "robotwin32") for t in DOMINO_35_TASKS]
_RANDOM_STATIC = [(f"Randomized/{t}", 1.0, "robotwin32") for t in DOMINO_35_TASKS]
...
DATASET_NAMED_MIXTURES = {
    ...
    "domino_wrap": _CLEAN_DYNAMIC_WRAP + _RANDOM_DYNAMIC_WRAP,
    "domino_clean_wrap": _CLEAN_DYNAMIC_WRAP,
    "domino_random_wrap": _RANDOM_DYNAMIC_WRAP,
