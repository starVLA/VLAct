...
History support
---------------
By default the client sends only the **current frame** to the policy server. If
the checkpoint config contains ``datasets.vla_data.history_frames > 0``, the
client sends raw temporal frames in the same order as training, e.g.
``t-8, t-4, t`` when ``history_frames=2`` and ``history_frame_gap=4``.

The older ``history_k`` pipeline is still available for experiments:
...
The server-side model consumes raw temporal frames through ``example["image"]``;
``history_images`` is passed separately only for legacy/custom history-aware
models.
"""
...
class ModelClient:
    ...
    def __init__(
        ...
        print(f"*** joint_action_unnorm_mode: {self.joint_action_unnorm_mode} ***")
        ...
        self.history_frame_gap = max(1, int(history_frame_gap))
        history_buffer_len = self.history_frames * self.history_frame_gap
        self.temporal_image_buffer: deque[List[np.ndarray]] = deque(maxlen=history_buffer_len)
        if self.history_frames > 0:
            print(
                f"*** raw temporal frames enabled: history_frames={self.history_frames}, "
                f"history_frame_gap={self.history_frame_gap} ***"
            )
    ...
    def reset(self, task_description: str) -> None:
        ...
        self.temporal_image_buffer.clear()
    ...
    def _build_temporal_images(self, current_images: List[np.ndarray]) -> List[np.ndarray]:
        if self.history_frames <= 0:
            return current_images

        images: List[np.ndarray] = []
        buffered_frames = list(self.temporal_image_buffer)
        buffer_len = len(buffered_frames)
        for history_idx in range(self.history_frames, 0, -1):
            offset = history_idx * self.history_frame_gap
            if offset > buffer_len:
                continue
            images.extend(buffered_frames[buffer_len - offset])
    ...
    def step(
        ...
        example["image"] = self._build_temporal_images(images)
        ...
        self.temporal_image_buffer.append(list(images))
    ...
    def unnormalize_actions(
        ...
        normalized_actions = np.clip(raw_actions, -1, 1)
        if joint_action_unnorm_mode == "wrap":
            actions = np.array(normalized_actions, copy=True)
...
def get_model(usr_args: dict) -> ModelClient:
    ...
    *usr_args*.  History settings are optional and default to disabled.
    ...
    history_frames, history_frame_gap = _get_model_history_config(policy_ckpt_path)
    history_frames = int(usr_args.get("history_frames", history_frames))
    history_frame_gap = int(usr_args.get("history_frame_gap", history_frame_gap))
    ...
    return ModelClient(
        ...
        history_frame_gap=history_frame_gap,
        action_chunk_size=usr_args.get("action_chunk_size"),
    )
...
def _get_model_image_size(policy_ckpt_path) -> tuple[list[int], Optional[list[list[int]]]]:
    ...
    image_size = vla_data_cfg.get("image_size") or vla_data_cfg.get("obs_image_size") or [224, 224]
    image_size = [int(image_size[0]), int(image_size[1])]
    print(f"*** image_size: {image_size} ***")
    return image_size, None


def _get_model_history_config(policy_ckpt_path) -> tuple[int, int]:
    model_config, _ = read_mode_config(Path(policy_ckpt_path))
    vla_data_cfg = ((model_config.get("datasets") or {}).get("vla_data") or {})
    history_frames = int(vla_data_cfg.get("history_frames", 0) or 0)
    history_frame_gap = int(vla_data_cfg.get("history_frame_gap", 1) or 1)
    history_frames = max(0, history_frames)
    history_frame_gap = max(1, history_frame_gap)
    return history_frames, history_frame_gap
