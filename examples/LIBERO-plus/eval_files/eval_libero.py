...
def eval_libero(args: Args) -> None:
    ...
    task_id_start = 0 if args.task_id_start < 0 else args.task_id_start
    ...
    if task_id_start >= task_id_end:
        raise ValueError(
            f"Invalid task range [{task_id_start}, {task_id_end}) for {args.task_suite_name}"
        )
    ...
    with open(os.path.join(args.log_path, f'{task_id_start}_to_{task_id_end}.json'), 'w', encoding='utf-8') as f:
