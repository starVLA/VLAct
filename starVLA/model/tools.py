...
def collate_fn_extend_dim(batch, max_dim=32):
    for sample in batch:
        if "action" in sample and sample["action"].shape[-1] < max_dim:
            pad_width = max_dim - sample["action"].shape[-1]
            sample["action"] = np.concatenate(
                [sample["action"], np.zeros((sample["action"].shape[0], pad_width), dtype=sample["action"].dtype)],
                axis=-1,
            )
        if "state" in sample and sample["state"].shape[-1] < max_dim:
            pad_width = max_dim - sample["state"].shape[-1]
            sample["state"] = np.concatenate(
                [sample["state"], np.zeros((sample["state"].shape[0], pad_width), dtype=sample["state"].dtype)],
                axis=-1,
            )
    return batch
