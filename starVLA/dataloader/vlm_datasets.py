...
def split_into_blocks(items, block_size=100):
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, tokenizer: transformers.PreTrainedTokenizer, data_args):
        ...
        for data in dataset_list:
            file_format = data.get("file_format", data["annotation_path"].split(".")[-1])
            if file_format == "parquet":
                sample_blocks.extend(self._build_parquet_blocks(data))
            ...
            annotations = self.pre_filter_long_case(annotations, max_words=tokenizer.max_len_single_sentence)
            sample_blocks.extend(split_into_blocks(annotations))

        random.shuffle(sample_blocks)  # Shuffle at block level to keep parquet row-group reads local.
        list_data_dict = list(itertools.chain.from_iterable(sample_blocks))

        self.tokenizer = tokenizer
        self.list_data_dict = list_data_dict
        self.data_args = data_args
        self._parquet_cache_key = None
        self._parquet_cache_rows = None
    ...
    def _build_parquet_blocks(self, data):
        try:
            import pyarrow.parquet as pq
        except ImportError as exc:
            raise ImportError("pyarrow is required for parquet VLM datasets") from exc

        annotation_path = Path(data["annotation_path"])
        if annotation_path.is_dir():
            parquet_paths = sorted(annotation_path.glob("*.parquet"))
        else:
            parquet_paths = [annotation_path]
        if not parquet_paths:
            raise ValueError(f"No parquet files found under {annotation_path}")

        sampling_rate = data.get("sampling_rate", 1.0)
        parquet_blocks = []
        for parquet_path in parquet_paths:
            path_idx = len(self.parquet_paths)
            self.parquet_paths.append(str(parquet_path))
            parquet_file = pq.ParquetFile(parquet_path)
            for row_group_idx in range(parquet_file.metadata.num_row_groups):
                row_count = parquet_file.metadata.row_group(row_group_idx).num_rows
                block = []
                for row_idx in range(row_count):
                    if sampling_rate < 1.0 and random.random() > sampling_rate:
                        continue
                    block.append(("parquet", path_idx, row_group_idx, row_idx))
                if block:
                    parquet_blocks.append(block)
        return parquet_blocks

    def _load_parquet_sample(self, parquet_ref):
        try:
            import pyarrow.parquet as pq
        except ImportError as exc:
            raise ImportError("pyarrow is required for parquet VLM datasets") from exc

        _, path_idx, row_group_idx, row_idx = parquet_ref
        cache_key = (path_idx, row_group_idx)
        if self._parquet_cache_key != cache_key:
            parquet_file = pq.ParquetFile(self.parquet_paths[path_idx])
            table = parquet_file.read_row_group(
                row_group_idx,
                columns=["id", "image", "conversations"],
                use_threads=False,
            )
            self._parquet_cache_rows = table.to_pylist()
            self._parquet_cache_key = cache_key

        row = self._parquet_cache_rows[row_idx]
        image = row.get("image") or {}
        image_bytes = image.get("bytes") if isinstance(image, dict) else image
        if not image_bytes:
            raise ValueError(f"No image bytes in parquet sample {self.parquet_paths[path_idx]}:{row_group_idx}:{row_idx}")
        return {
            "id": row.get("id"),
    ...
    def lengths(self):
        ...
        for sample in self.list_data_dict:
            if isinstance(sample, tuple) and sample[0] == "parquet":
    ...
    def pre_calculated_length(self):
        first_sample = self.list_data_dict[0]
        if not isinstance(first_sample, tuple) and "num_tokens" in first_sample:
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if isinstance(raw_item, tuple) and raw_item[0] == "parquet":
            raw_item = self._load_parquet_sample(raw_item)
        sources = raw_item
        if isinstance(i, int):
            sources = [sources]
        assert len(sources) == 1, "Don't know why it is wrapped to a list"  # FIXME
        video = None
        has_image_bytes = "image_bytes" in sources[0]
        if has_image_bytes:
            image, grid_thw = self.process_image_bytes(sources[0]["image_bytes"])
            image = [image]
        ...
        elif "images" in sources[0] and len(sources[0]["images"]):
        ...
        if has_image_bytes or "images" in self.list_data_dict[i]:
