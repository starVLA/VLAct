...
class VideoTransform(ModalityTransform):
    ...
    def set_metadata(
        ...
        super().set_metadata(dataset_metadata, original_metadata=original_metadata)
...
class VideoResizeBucketedSizes(VideoTransform):
    target_sizes: list[tuple[int, int]] = Field(
        ...,
        description="Candidate resize sizes as (width, height); the closest aspect ratio is selected",
    )
    interpolation: str = Field(default="linear", description="The interpolation mode")
    antialias: bool = Field(default=True, description="Whether to apply antialiasing")
    ...
    def validate_target_sizes(cls, v):
        if not v:
            raise ValueError("target_sizes must not be empty")
        normalized_sizes = []
        for size in v:
            if len(size) != 2:
                raise ValueError(f"each target size must have 2 elements, got {size}")
    ...
    def set_metadata(
        ...
        ModalityTransform.set_metadata(self, dataset_metadata, original_metadata=original_metadata)
        self.original_resolutions = {}
        for key in self.apply_to:
            split_keys = key.split(".")
            assert len(split_keys) == 2, f"Invalid key: {key}. Expected format: modality.key"
    ...
    def get_transform(self, mode: Literal["train", "eval"] = "train") -> Callable:
        ...
        if interpolation is None:
            raise ValueError(
                f"Interpolation mode {self.interpolation} not supported for {self.backend}"
            )

        if self.backend == "torchvision":
            def _resize_torch(frames: torch.Tensor) -> torch.Tensor:
                target_height, target_width = self._compute_target_size(
                    height=int(frames.shape[-2]),
                    width=int(frames.shape[-1]),
                )
                if target_height == frames.shape[-2] and target_width == frames.shape[-1]:
                    return frames
                return T.Resize(
            ...
            return _resize_torch

        if self.backend == "albumentations":
            def _resize_numpy(frames: np.ndarray) -> np.ndarray:
                if frames.ndim == 5:
                    return np.stack([_resize_numpy(sample) for sample in frames], axis=0)
                target_height, target_width = self._compute_target_size(
                    height=int(frames.shape[-3]),
                    width=int(frames.shape[-2]),
                )
                if target_height == frames.shape[-3] and target_width == frames.shape[-2]:
                    return frames
                return np.stack(
                    [
    ...
    def apply(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        # Resize each view independently so mixed-resolution camera streams can be handled.
        for key in self.apply_to:
...
class VideoToTensor(VideoTransform):
    ...
    def check_input(self, data: dict):
        ...
        for key in self.apply_to:
            ...
            if input_resolution != expected_resolution:
                # Trust decoded frame geometry over stale metadata (e.g. H264 vs ffprobe mismatch).
                self.original_resolutions[key] = input_resolution
                expected_resolution = input_resolution
            assert (
