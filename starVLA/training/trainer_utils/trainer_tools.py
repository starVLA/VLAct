...
class TrainerUtils:
    ...
    @staticmethod
    def load_pretrained_backbones(model, checkpoint_path=None, reload_modules=None, ignore_modules=None):
        """
        load checkpoint:
        - if reload_modules is set, load by path part
        - if ignore_modules is set, skip loading these module paths from checkpoint
        ...
        ignore_prefixes = []
        if ignore_modules:
            ignore_prefixes = [p.strip() for p in ignore_modules.split(",") if p.strip()]
            checkpoint = {
                k: v
                for k, v in checkpoint.items()
                if not any(k == prefix or k.startswith(prefix + ".") for prefix in ignore_prefixes)
            }
            if dist.get_rank() == 0 and ignore_prefixes:
                print(f"⏭️ skip loading checkpoint params for modules: {', '.join(ignore_prefixes)}")
